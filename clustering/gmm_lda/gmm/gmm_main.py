    ...
    def __init__(self, dim = None, ncomps = None, data = None,  method = None, filename = None, params = None):
        ...
        if not filename is None:  # load from file
            ...
            if method == "uniform":
                ...
                (centroids, labels) = vq.kmeans2(data, ncomps, minit="points", iter=10)
                
                # IMPORTANT: check if there have abnormal clusters
                for i in range(ncomps):
                    if any(centroids[i,:]):
                        continue
                    else:
                        label_count = Counter(labels)
                        top_label = np.argmax(label_count)
                        randnum = pr.randint(0,label_count[top_label])
                        label_pos = [j for j,v in enumerate(labels) if v==1]
                        pos = label_pos[randnum]                
                        centroids[i,:] = data[pos,:]
                        labels[pos] = i
    ...
    def em(self, data, nsteps = 100):
        ...
        for l in range(nsteps):
            ...
            for j in range(n):
                for i in range(k):
                    pri = self.priors[i]
                    pri = np.maximum(np.array(pri), 1e-10)
                    com = self.comps[i].pdf(data[j])
                    com = np.maximum(np.array(com), 1e-10)
                    responses[i,j] = pri * com
            ...
            label_count = []
            for i in range(k):
                label = np.argmax(responses, axis=0)
                label_count.append(np.size(label[label==i]))
            ...
            for i in range(k):
                ...
                self.priors[i] = N[i] / np.sum(N) # normalize the new priors,pi
